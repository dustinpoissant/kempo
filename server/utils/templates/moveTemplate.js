...
export default async ({ rootDir, file, newFile }) => {
  ...
  if(meta.locked === 'true'){
    return [{ code: 403, msg: 'Cannot move a locked template' }, null];
  }
  if(meta.owner && meta.owner !== 'custom'){
    return [{ code: 403, msg: 'Cannot move system templates' }, null];
  }

  if(existsSync(fullNewPath)){
    return [{ code: 409, msg: 'A template already exists at the new location' }, null];
  }
