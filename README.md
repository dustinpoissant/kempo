...
This processes all files in `src/` and outputs to `dist/`:
- JavaScript files are minified with terser, mangled, and source maps are generated
- HTML files are minified while preserving CMS front matter comments (important for page templating)
- JSON files are minified
- SVG icons and other assets are copied as-is

The build achieves ~15% size reduction and runs in ~0.6s.

**Output Structure:**
```
dist/
├── admin/           # Minified admin portal
├── kempo/           # Minified framework SDK, API routes, components
└── ...
```

See [spec/concepts/build.md](spec/concepts/build.md) for detailed build documentation.
...
kempo/
├── src/
│   ├── admin/                # Admin portal source (minified to dist/admin/)
│   └── kempo/                # Framework SDK & API source (minified to dist/kempo/)
├── server/                   # Backend utilities (not minified)
├── middleware/               # Auth middleware
├── app-public/               # Consumer boilerplate (not minified)
├── app-templates/            # Email and page templates (not minified)
├── app-server/               # Server scaffolding (not minified)
└── dist/                     # Built & minified output (generated by `npm run build`)
```

**Consumer project (created by `npx kempo init`):**
```
my-site/
├── public/                   # Copied from kempo's app-public/
├── server/db/schema.js       # Copied from kempo's app-server/
├── templates/                # Copied from kempo's app-templates/
└── ...
```
